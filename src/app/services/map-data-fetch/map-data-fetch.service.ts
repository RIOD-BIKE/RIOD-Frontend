...
//IMPORTANT -- IMPORTANT
  ...
  retrieveClusters():BehaviorSubject<Array<GeoCluster>> {   
    ...
    this.auth.getUserUID().toPromise().then(uid=>{ 
      console.log(uid);     
      let clusterCollection =this.db.collection('users').doc(uid).collection('clusters');  
  ...
  retrieveAssemblyPoints():BehaviorSubject<Array<GeoAssemblyPoint>> {       
    this.auth.getUserUID().toPromise().then(uid=>{ 
      console.log(uid);   
      let apCollection =this.db.collection('users').doc(uid).collection('assemblyPoints');  
  ...
  let recentRoutes: Array<RouteCl> = [new RouteCl(52.274557, 8.047160,"First Route"),new RouteCl(52.274557, 8.047160,"Second Route"),new RouteCl(52.274557, 8.047160,"Third Route"),new RouteCl(52.274557, 8.047160,"4 Route"),new RouteCl(52.274557, 8.047160,"Fifth Route")];
  }
